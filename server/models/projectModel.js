...
export async function updateSubtask({ taskId, subtaskId, requesterId, status, title }) {
  ...
  let normalizedTitle = title == null ? null : String(title || "").trim();
  ...
  if (rawStatus != null) {
    ...
    const explicitlyAllowedStatuses = ["finished", "unfinished"];
    if (explicitlyAllowedStatuses.includes(lowerRaw)) {
      chosenStatus = lowerRaw;
    } else {
      const error = new Error(`Invalid status value: "${rawStatus}". Allowed values are 'finished' or 'unfinished'.`);
      error.code = "INVALID_STATUS";
      throw error;
    }
  }

  if (chosenStatus != null && chosenStatus.length > 20) {
  ...
  const queryParams = [normalizedSubtaskId, normalizedTaskId];
  let paramIndex = 3;

  if (chosenStatus != null) {
    updateSetClauses.push(`status = $${paramIndex}::text`);
    queryParams.push(chosenStatus);
    paramIndex++;
  }
  if (normalizedTitle != null && normalizedTitle !== "") {
    updateSetClauses.push(`title = $${paramIndex}::text`);
    queryParams.push(normalizedTitle);
    paramIndex++;
  }

  // No valid fields — return current row unchanged
  if (updateSetClauses.length === 0) {
    const currentResult = await pool.query(
      `SELECT id, task_id, title, created_by, status, created_at
       FROM subtasks WHERE id = $1::int AND task_id = $2::int`,
      [normalizedSubtaskId, normalizedTaskId]
    );
    ...
    const r = currentResult.rows[0];
    return { id: r.id, taskId: r.task_id, title: r.title, createdBy: r.created_by, status: r.status, createdAt: r.created_at };
  }
  ...
  try {
    result = await pool.query(
      ...
      console.log("[updateSubtask model] updateSetClauses:", updateSetClauses);
      console.log("[updateSubtask model] queryParams:", queryParams);
      console.log("[updateSubtask model] result.rows:", result.rows);
      console.log("[updateSubtask model] rowCount:", result.rowCount);
  } catch (dbErr) {
    if (dbErr && (dbErr.code === "23514" || String(dbErr.constraint || "").toLowerCase().includes("status"))) {
      const e = new Error(`Invalid status value: "${rawStatus}". Allowed: finished, unfinished`);
  ...
  return {
    ...
    title: row.title,
    createdBy: userRow
      ? { id: userRow.id, firstName: userRow.first_name, lastName: userRow.last_name, email: userRow.email }
      : row.created_by,
    status: row.status,
    createdAt: row.created_at,
  };
...
export async function updateTaskName({ taskId, requesterId, name }) {
  ...
  if (!access.isOwner) {
    ...
    } else if (!access.isAssignee && !access.settings.allow_member_edit_task) {
